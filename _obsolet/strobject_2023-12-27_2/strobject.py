...
for the individual object types in python.

Walk through nested objects in create treestructure.
Main function is: '.str_object()'

It turned out to be very time-consuming!
RecursionErrors!
...
is_nested
    Check if given object is nested

__str_lvl_pad
    Returns string of given length width given fill charachters
    Generat string elements for string layout
...
def __str_value(level=int(), obj=None, strcontainerkey=True, container_key=str(), strtype=True, fillchar="·") -> str:
    """
    Create a str representativ of the node
    """
...
def str_object(obj=None, strcontainerkey=True, strtype=True, fillchar=None) -> str:
    """
    Main function!
    Walk and trough nested objects ...
    """
    if fillchar == None:
        fillchar = "·"
    stack = [obj, ]
